...
/* ── shared styles ── */
const inputCls =
  "w-full rounded-[10px] border border-black/10 bg-[#fafaf7] px-3 py-2 text-[12px] text-[#2d2d2d] transition placeholder:text-black/28 focus:border-[#ffd239] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#ffd239]/10";
const labelCls =
  "block text-[9px] font-bold uppercase tracking-[0.18em] text-black/34 mb-1";
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed bottom-[88px] right-4 z-50 w-[calc(100vw-32px)] max-w-[360px] overflow-hidden rounded-[18px] border border-black/8 bg-white shadow-[0_16px_50px_rgba(0,0,0,0.13)] sm:right-8">
          ...
          {sent ? (
            ...
            <div className="px-5 py-6 text-center">
              <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full" style={{ background: "rgba(37,211,102,0.10)" }}>
                <WaIcon className="h-5 w-5" style={{ color: "#25d366" }} />
              </div>
              <p className="text-[11px] font-semibold text-brand-charcoal">
                WhatsApp abierto
              </p>
              ...
              <button
                ...
                className="mt-4 text-[10px] font-semibold uppercase tracking-[0.14em] text-black/30 underline underline-offset-2 transition hover:text-brand-charcoal"
              >
                Nueva consulta
              </button>
            </div>
            ...
            /* form */
            <form onSubmit={handleSubmit(onSubmit)} className="divide-y divide-black/[0.05]">
              ...
              <div className="px-4 py-3.5">
                ...
                <Select.Root value={interest} onValueChange={(v) => setValue("interest", v, { shouldValidate: true })}>
                  <Select.Trigger
                    className={cn(
                      "flex w-full items-center justify-between rounded-[10px] border px-3 py-2 text-[12px] outline-none transition-all",
                      "data-[state=open]:border-[#ffd239] data-[state=open]:ring-2 data-[state=open]:ring-[#ffd239]/14",
                      errors.interest
                        ? "border-red-300 bg-red-50/40"
                        : interest
                        ? "border-[#ffd239]/60 bg-white font-medium text-brand-charcoal"
                        : "border-black/10 bg-[#fafaf7] text-black/30 hover:border-black/18",
                    )}
                  >
                  ...
                  <Select.Portal>
                    <Select.Content
                      position="popper"
                      sideOffset={4}
                      className="z-[60] w-[var(--radix-select-trigger-width)] overflow-hidden rounded-[13px] border border-black/8 bg-white shadow-[0_10px_36px_rgba(0,0,0,0.13)]"
                    >
                      <Select.Viewport className="p-1.5">
                        {contactInterestOptions.map((opt) => (
                          <Select.Item
                            key={opt}
                            value={opt}
                            className="flex cursor-pointer select-none items-center justify-between rounded-[8px] px-3 py-2 text-[11px] text-black/56 outline-none transition hover:bg-[#fafaf7] hover:text-brand-charcoal data-[state=checked]:bg-[rgba(255,210,57,0.10)] data-[state=checked]:font-semibold data-[state=checked]:text-brand-charcoal"
                          >
                ...
                {errors.interest && (
                  <p className="mt-0.5 text-[10px] text-red-500">{errors.interest.message}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5 px-4 py-3.5">
                <div>
                  ...
                  <input id="wa-name" type="text" className={inputCls} placeholder="Tu nombre" {...register("name")} />
                ...
                <div>
                  ...
                  <input id="wa-phone" type="tel" className={inputCls} placeholder="+54 9 ..." {...register("phone")} />
                </div>
              </div>

              {/* mensaje */}
              <div className="px-4 py-3.5">
                <label htmlFor="wa-message" className={labelCls}>Mensaje breve</label>
                <textarea
                  id="wa-message"
                  rows={2}
